...
<header x-data="{ openMenuIndex: null, open: false, activeParentIndex: null }"
    class="flex sticky bg-white  w-full z-[90] z-100 h-[80px] top-0 items-center py-4 lg:py-0 border-b-gray-200 border-b"
    id="header">
            ...
            <div class="gap-6 flex items-stretch relative " id="mega_menu_container">
                {{-- @foreach (collect(config('menu'))->where('display', true) as $index => $menuItem)
                    ...
                    relative flex items-center"
                        x-data="{ isHovered: false }" @mouseenter="openMenuIndex = {{ $index }}; isHovered = true"
                        @mouseleave="openMenuIndex = null; isHovered = false">
                        @if (!isset($menuItem['children']) || (isset($menuItem['children']) && is_array($menuItem['children']) && count($menuItem['children']) == 0))
                            <div class="menu-link-header border-b border-b-transparent">
                                <a wire:navigate
                                    href="{{ get_route_object_link($menuItem['link'] ?? null) }}"
                                    class="menu-title-header hover:text-gray-500 text-[.8125rem] leading-[1.125rem] text-gray-800 font-medium">
                                    {!! $menuItem['label'] !!}
                                </a>
                            </div>
                        @else
                ...
                @foreach (collect(get_menu())->where('display', true) as $index => $menuItem)
                    ...
                    relative flex items-center"
                        ...
                        @if (
                            ...
                            <div class="menu-link-header border-b border-b-transparent">
                                <a wire:navigate href="{{ $menuItem['link'] ?? null }}"
                                    class="menu-title-header hover:text-gray-500 text-[.8125rem] leading-[1.125rem] text-gray-800 font-medium">
                                    {!! $menuItem['label'] !!}
                                </a>
                            </div>
                        ...
                        <div>
                            ...
                            @if ($cms_user)
                                <div class="relative cursor-pointer h-full">
                                    ...
                                    <div x-cloak x-transition:enter.duration.500ms x-transition:leave.duration.400ms
                                        x-show="open" class="menu-dropdown menu-initials-container  ">
